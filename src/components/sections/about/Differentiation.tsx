  ...
  return (
    <section className="relative overflow-hidden bg-black px-6 py-20 md:px-16">
      ...
      <div className="relative mx-auto max-w-[1024px]">
        ...
        <div className="mt-16 grid overflow-hidden border bg-white/95 dark:bg-slate-900/95 md:grid-cols-2 transition-colors" style={{ borderRadius: radiuses.pill, borderColor: isDark ? "rgba(255,255,255,0.1)" : "rgba(65,71,83,0.1)" }}>
          <ListBlock title="Standard AI Tools" icon="/section-4-about/diff-cross.svg" items={LEFT} accent={isDark ? "#94A3B8" : "#414753"} />
          <ListBlock title="Shekel Ecosystem" icon="/section-4-about/diff-check.svg" items={RIGHT} accent={isDark ? "#3B82F6" : "#005ab6"} />
        </div>
      </div>
    </section>
  );
  ...
  return (
    <div className="p-10">
      <div className="mb-8 flex items-center gap-2">
        <Image src={icon} alt="" width={20} height={20} className={isDark && title.includes("Standard") ? "brightness-200" : ""} />
      ...
      <ul className="space-y-6">
        {items.map((item, idx) => (
          <li key={item} className="flex gap-4">
            ...
            <span className="text-black dark:text-white" style={{ fontFamily: typography.fonts.inter, fontWeight: 400, fontSize: 16, lineHeight: "24px" }}>{item}</span>
          </li>
        ))}
      </ul>
    </div>
