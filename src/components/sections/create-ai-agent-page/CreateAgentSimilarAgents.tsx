    ...
    <section className="w-full">
      <div className="w-full px-4 md:px-8">
        <div className="flex items-end justify-between">
          ...
          <Link href="/marketplace" className="hidden items-center gap-1 uppercase md:flex" style={{ color: "#000", letterSpacing: "1.4px", fontSize: 14 }}>
            Explore More Agents
        ...
        <div className="mt-10 grid gap-5 md:grid-cols-3 md:gap-10">
          {CARDS.map((c) => (
            <div key={c.name} className="rounded-2xl p-8" style={{ background: "#f2f4f7" }}>
              ...
              <Link href="/agent-detail" className="rounded-xl bg-white py-3 text-center font-semibold" style={{ display: 'block', color: "#191c1e", border: "1px solid rgba(193,198,213,0.1)", borderRadius: radiuses.inputIcon }}>
                Run Agent
              </Link>
            </div>
          ))}
